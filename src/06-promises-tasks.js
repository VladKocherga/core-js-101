...
function willYouMarryMe(isPositiveAnswer) {
  ...
  return new Promise((resolve, rejerect) => {
    ...
    return rejerect(new Error('Wrong parameter is passed! Ask her again.'));
  });
}
...
function chainPromises(array, action) {
  return new Promise((res) => {
    const answers = [];
    array.forEach((item) => item.then((answer) => answers.push(answer)));
    res(answers);
